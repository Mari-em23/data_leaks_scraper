# Run from /Code : python -m Forums.darkforums.search_and_scrape_from_darkforums
...
"""
def main():
    url_search = "https://darkforums.me/search.php"
    ...
    with sync_playwright() as p:
        ...
        page = context.new_page() # Creates a new tab in the browser for interacting with the website
        ...
        try:
            ...
            # Fill in search form
            page.wait_for_selector('input.textbox[name="keywords"][size="35"]', timeout=5000)
            page.fill('input.textbox[name="keywords"][size="35"]', keyword)
            
            # Submit search
            page.locator('input.button[name="submit"][value="Search"]').first.click()
            ...
            while True:
                ...
                for row in rows:
                    ...
                    replies = 'N/A'
                    views = 'N/A'
                    if len(replies_cells) >= 4: 
                        replies_text = replies_cells[3].text_content().strip()
                        replies = replies_text if replies_text else 'N/A'

                    if len(replies_cells) >= 5: 
                        views_text = replies_cells[4].text_content().strip()
                    ...
                    date_str = post_date_text.split('(')[0].strip()
            ...
            try:
                err_path = os.path.join('Forums/darkforums/search_error.png')
